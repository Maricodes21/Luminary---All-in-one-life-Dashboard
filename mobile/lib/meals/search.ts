...
export async function searchFoods(query: string, locale = 'en-ZA'): Promise<FoodSearchResult[]> {
  ...
  try {
    ...
    const parsed = foodSearchResultSchema.array().safeParse(normalizeGatewayResults(data?.data?.results ?? data?.results ?? []));
    if (!parsed.success) throw new Error('Food search returned an invalid result.');
    return mergeResults(parsed.data, local);
  } catch (error) {
    console.warn('[meals] Live food search unavailable; showing verified local results', error instanceof Error ? error.message : error);
    return local;
  }
}
...
export async function analyzeMealPhoto(base64: string): Promise<FoodSearchResult[]> {
  const { data, error } = await supabase.functions.invoke('meals-api', {
    body: { action: 'analyze-meal-photo', input: { imageBase64: base64, mimeType: 'image/jpeg', locale: 'en-ZA' } },
  });
  if (error) throw error;
  const parsed = foodSearchResultSchema.array().safeParse(normalizeGatewayResults(data?.data?.results ?? data?.results ?? []));
  if (!parsed.success) throw new Error('The meal analysis could not be verified.');
  return parsed.data;
}
...
export async function submitCommunityFood(input: {
  ...
  nutrition: { calories: number; proteinG: number | null; carbsG: number | null; fatG: number | null };
}) {
  const { data, error } = await supabase.functions.invoke('meals-api', { body: { action: 'submit-food', input } });
  if (error) throw error;
...
export function localFoodResults(query: string): FoodSearchResult[] {
  ...
  return getAllLibraryMeals()
    .filter((meal) => !terms.length || terms.every((term) => `${meal.name} ${meal.ingredients.join(' ')} ${meal.tags.join(' ')}`.toLowerCase().includes(term)))
    .map((meal) => ({
      id: meal.id,
      name: meal.name,
      ...
      confidence: 1,
      nutrition: { calories: meal.calories, proteinG: meal.proteinG, carbsG: meal.carbsG, fatG: meal.fatG },
      servings: [{
        id: `${meal.id}_serving`, quantity: 1, unit: 'serving', label: '1 serving', providerId: meal.source.sourceId,
        nutrition: { calories: meal.calories, proteinG: meal.proteinG, carbsG: meal.carbsG, fatG: meal.fatG },
      }],
    }));
}
