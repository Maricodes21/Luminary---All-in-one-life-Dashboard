...
export type MoodEstimateInput = {
  ...
  listeningReaction?: MoodLabel | null;
...
export function estimateMoodLocally(input: MoodEstimateInput): MoodEstimate | null {
  ...
  const add = (
    label: MoodLabel,
    weight: number,
    family: MoodEstimate['contributingFamilies'][number],
  ) => {
    scores.set(label, (scores.get(label) ?? 0) + weight);
    families.add(family);
  };

  input.confirmedMoodHistory
    .slice(-3)
    .forEach((item, index) => add(item.label, 0.52 - index * 0.08, 'confirmed_mood'));
  if (input.listeningReaction) add(input.listeningReaction, 0.62, 'listening_reaction');
  input.journal?.tags.forEach((tag) => {
    const label = tagMap[tag.trim().toLowerCase()];
    if (label) add(label, 0.5, 'journal');
  });
  const ratio = input.commitments.scheduled
    ? input.commitments.completed / input.commitments.scheduled
    : null;
  if (ratio !== null && ratio >= 0.8) add('grounded', 0.3, 'commitments');
  if (ratio !== null && ratio <= 0.34 && input.commitments.scheduled >= 3)
    add('cloudy', 0.22, 'commitments');
  if (input.movement?.workoutCompleted) add('energized', 0.28, 'movement');
  if ((input.movement?.steps ?? 0) >= 8_000) add('energized', 0.2, 'movement');
  if ((input.meals?.loggedMealCount ?? 0) >= 3) add('grounded', 0.15, 'meals');
  if (input.ritual.recentCompletionRate >= 0.75) add('reflective', 0.15, 'ritual');
  if (input.localHour >= 21) add('reflective', 0.08, 'time');
  ...
  const confidence = Math.min(
    0.92,
    0.42 + (top / Math.max(total, 0.01)) * 0.42 + Math.min(families.size, 4) * 0.025,
  );
...
function explanationFor(families: MoodEstimate['contributingFamilies']) {
  const names: Record<MoodEstimate['contributingFamilies'][number], string> = {
    confirmed_mood: 'moods you confirmed before',
    listening_reaction: 'how you said the listening felt',
    journal: 'your journal tags',
    commitments: 'today’s commitments',
    movement: 'movement you recorded',
    meals: 'meal timing',
    ritual: 'your ritual rhythm',
