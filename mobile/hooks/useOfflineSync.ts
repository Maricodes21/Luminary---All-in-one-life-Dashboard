 ...
 *
 * Mount this once in the app root so queued writes are not dependent on
 * visiting a particular module. Replays are scoped to the signed-in owner.
...
export function useOfflineSync(userId: string | null): void {
  ...
  useEffect(() => {
    ...
    void flushQueue(userId).then(({ flushed, remaining }) => {
      if (flushed > 0) {
    ...
    const sub = AppState.addEventListener('change', (nextState) => {
      ...
      if (prev.match(/inactive|background/) && nextState === 'active') {
        void flushQueue(userId).then(({ flushed, remaining }) => {
          if (flushed > 0) {
            console.log(
              `[offlineSync] foreground flush: ${flushed} written, ${remaining} remaining`,
            );
          }
        });
      }
