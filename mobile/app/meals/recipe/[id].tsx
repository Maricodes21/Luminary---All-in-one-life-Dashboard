...
export default function RecipeDetailScreen() {
  ...
  return (
    <MealScreen title={recipe.name} subtitle={`${recipeDuration(recipe)} min / ${recipe.servings} ${recipe.servings === 1 ? 'serving' : 'servings'}`}>
      ...
      {entryId ? <Pressable onPress={() => router.push({ pathname: '/meals/substitute', params: { id: entryId } })} style={styles.secondary}><Icon name="swap" size={18} color={palette.primary} /><Text style={[type.labelMd, { color: palette.primary }]}>Substitute meal</Text></Pressable> : null}
