  ...
  return (
    <MealScreen title={`Edit ${formatDay(localDate)}`} subtitle="Shape this day without rebuilding the week">
      <View style={styles.section}>
        ...
        {entries.map((entry) => <DynamicMealCard key={entry.id} title={entry.name} recipeId={entry.recipeId} imageUri={entry.imageUri ?? recipeImageUri(entry)} nutrition={entry.nutrition} detail={titleCase(entry.mealType)} onPress={() => router.push({ pathname: '/meals/recipe', params: { id: entry.recipeId ?? 'missing', entryId: entry.id } })} onEdit={() => router.push({ pathname: '/meals/substitute', params: { id: entry.id } })} onDelete={() => removeEntry(entry.id, entry.name)} />)}
      ...
      <View style={styles.section}>
        ...
        {candidates.map((recipe) => <View key={recipe.id} style={styles.candidate}><DynamicMealCard title={recipe.name} recipeId={recipe.id} imageUri={recipeImageUri(recipe)} nutrition={recipe.nutrition} detail={`${recipe.prepMinutes + recipe.cookMinutes} min`} onPress={() => router.push({ pathname: '/meals/recipe', params: { id: recipe.id } })} /><Pressable onPress={() => addRecipe(recipe)} style={styles.addButton} accessibilityRole="button" accessibilityLabel={`Add ${recipe.name} to ${formatDay(localDate)}`}><Text style={[type.labelMd, { color: palette.onPrimary }]}>Add meal</Text></Pressable></View>)}
