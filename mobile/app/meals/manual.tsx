...
export default function ManualMealScreen() {
  ...
  const existing = useMemo(
    () => user?.meals.find((meal) => meal.id === params.id),
    [params.id, user?.meals],
  );
  ...
  const [calories, setCalories] = useState(
    existing?.nutrition.calories?.toString() ?? params.calories ?? '',
  );
  const [protein, setProtein] = useState(
    existing?.nutrition.proteinG?.toString() ?? params.protein ?? '',
  );
  ...
  const [mealType, setMealType] = useState<MealType>(
    existing?.mealType ??
      (isMealType(params.mealType) ? params.mealType : mealWindowFor(new Date())),
  );
  ...
  const save = () => {
    ...
    const macros = [
      parseOptionalNonnegative(protein),
      parseOptionalNonnegative(carbs),
      parseOptionalNonnegative(fat),
    ];
    if (!name.trim() || !calorieValue.valid) {
      Alert.alert(
        'Name and calories are required',
        'Macros can stay blank when you do not know them.',
      );
      return;
    }
    ...
    if (macros.some((macro) => !macro.valid)) {
      Alert.alert(
        'Check the macros',
        'Macro values must be zero or greater, or left blank when unknown.',
      );
      return;
    }
    const now = new Date();
    const record = {
      name: name.trim(),
      localDate: existing?.localDate ?? localDateKey(now),
      consumedAt: existing?.consumedAt ?? now.toISOString(),
      timezone: existing?.timezone ?? currentTimezone(),
      mealType,
      servingQuantity: quantityValue.value,
      servingUnit: unit.trim(),
      nutrition: {
        calories: calorieValue.value,
        proteinG: macros[0].value,
        carbsG: macros[1].value,
        fatG: macros[2].value,
      },
      source: (existing?.source ??
        (isMealSource(params.source) ? params.source : 'manual')) as MealSource,
      providerId: existing?.providerId ?? params.providerId,
      notes:
        [
          notes.trim(),
          params.source === 'grounded_web' && params.sourceUrl
            ? `Nutrition source: ${params.sourceUrl}${params.retrievedAt ? ` (retrieved ${params.retrievedAt})` : ''}`
            : '',
        ]
          .filter(Boolean)
          .join('\n') || undefined,
  ...
  return (
    <MealScreen
      title={existing ? 'Edit meal' : 'Manual entry'}
      subtitle="Add as much detail as you know"
      action={
    ...
    >
      {params.source === 'grounded_web' ? (
        <View style={styles.sourceBanner}>
          <Text style={[type.labelMd, { color: palette.onSurface }]}>
            Check this sourced estimate
          </Text>
          <Text style={[type.bodySm, { color: palette.onSurfaceVariant }]}>
            Nutrition was extracted from an online source, not invented by the model. Confirm the
            serving and values before saving.
          </Text>
          {params.sourceUrl ? (
            <Text style={[type.bodySm, { color: palette.primary }]} numberOfLines={2}>
              {params.sourceUrl}
            </Text>
          ) : null}
        </View>
      ...
      <View style={styles.typeRow}>
        {(['breakfast', 'lunch', 'dinner', 'snack'] as const).map((item) => (
          <Pressable
            key={item}
            onPress={() => setMealType(item)}
            style={[styles.typeButton, mealType === item && styles.active]}
          >
            <Text
              style={[
                type.labelSm,
                { color: mealType === item ? palette.onPrimary : palette.onSurfaceVariant },
              ]}
            >
              {item}
            </Text>
      ...
      <Field
        ...
        placeholder="Sauce on the side, half portion..."
        multiline
      />
      {imageUri && !imageFailed ? (
          ...
          onPress={() =>
            router.push({
              ...
              params: {
                ...
                returnUnit: unit,
                returnMealType: mealType,
                returnNotes: notes,
                returnProviderId: params.providerId,
        ...
        >
          <Icon name="camera" size={20} color={palette.primary} />
          <Text style={[type.labelMd, { color: palette.onSurface }]}>
            {imageFailed ? 'Take another photo' : 'Take a photo'}
          </Text>
        </Pressable>
      )}
    </MealScreen>
...
function Field({
  ...
  return (
    <View style={styles.field}>
      <Text style={[type.labelMd, { color: palette.onSurfaceVariant }]}>{label}</Text>
      <TextInput
        {...props}
        multiline={multiline}
...
function isMealSource(value?: string): value is MealSource {
  return (
    value === 'manual' ||
    value === 'curated' ||
    value === 'usda' ||
