...
export default function MealsScreen() {
  ...
  const todayMeals = useMemo(
    () => user?.meals.filter((meal) => meal.localDate === today) ?? [],
    [today, user?.meals],
  );
    ...
    <View style={styles.root}>
      ...
      >
        <View style={styles.header}>
          <View style={styles.headerCopy}>
            ...
            <Text style={[type.headlineLg, { color: palette.onSurface, marginTop: 2 }]}>
              Your day, fed well
            </Text>
          </View>
          <View style={styles.headerActions}>
            {mode === 'plan' && plan ? (
              ...
              >
                <Icon name="receipt" color={palette.primary} size={20} />
              </Pressable>
            ) : null}
            <Pressable
          ...
          <PlanMode
            ...
            onOpenRecipe={(entry) =>
              router.push({
                ...
                params: { id: entry.recipeId ?? 'missing', entryId: entry.id },
              })
            }
            onCreatePlan={() => router.push('/meals/plan-builder')}
            onEditEntry={(entry) =>
              router.push({ pathname: '/meals/substitute', params: { id: entry.id } })
            }
            onEditDay={() =>
              plan &&
              router.push({
                pathname: '/meals/edit-day',
      ...
      {user?.undo?.kind === 'meal' ? (
        <View style={[styles.undo, { bottom: insets.bottom + 82 }]}>
          <Text style={[type.bodySm, styles.undoText]} numberOfLines={1}>
            {user.undo.record.name} deleted
          </Text>
          <Pressable
            onPress={undoMealDeletion}
          ...
          <Pressable
            onPress={dismissMealDeletion}
            style={styles.undoDismiss}
            accessibilityRole="button"
            accessibilityLabel="Dismiss deleted meal notice"
          >
        ...
        <View style={styles.macroColumn}>
          ...
          <Pressable onPress={onOpenProfile} accessibilityRole="button">
            <Text style={[type.bodySm, { color: palette.primary, marginTop: spacing.sm }]}>
              {user?.profile
                ? `Based on ${user.profile.weightKg} kg  /  updated ${formatShortDate(user.profile.updatedAt)}`
                : 'Set your nutrition profile'}
            </Text>
          </Pressable>
        </View>
      </Card>
      ...
      {profileStale ? (
        <View style={styles.reminder}>
          <Pressable onPress={onOpenProfile} style={styles.reminderCopy} accessibilityRole="button">
            ...
            <Text style={[type.bodySm, { color: palette.onSurface, flex: 1 }]}>
              Has your weight changed? Refresh your targets when it feels useful.
            </Text>
          </Pressable>
          <Pressable
            onPress={onDismissStale}
            style={styles.dismiss}
            accessibilityLabel="Dismiss weight reminder"
          >
            <Icon name="close" size={16} color={palette.onSurfaceVariant} />
          </Pressable>
        </View>
        ...
        <View style={styles.cardList}>
          {meals.map((meal) => (
            <DynamicMealCard
              key={meal.id}
              title={meal.name}
              recipeId={catalogRecipeId(meal.providerId)}
              imageUri={meal.imageUri}
              nutrition={meal.nutrition}
              detail={`${titleCase(meal.mealType)}  /  ${formatTime(meal.consumedAt)}`}
          ...
          {!meals.length ? (
            <EmptyState
              ...
              detail="Search the full food library, scan a meal, or add exactly what you know."
            />
          ) : null}
        </View>
      </View>
...
function SmartSuggestion({
  ...
  user: ReturnType<typeof activeMealsUser>;
  target: NonNullable<ReturnType<typeof activeMealsUser>>['targets'][string] | null;
  remainingCalories: number | null;
  meals: MealLogRecord[];
  onOpenRecipe: (recipeId: string) => void;
  onLogRecipe: (recipeId: string) => void;
  ...
  const recentRecipeIds = useMemo(
    () =>
      user?.plans
        .flatMap((plan) =>
          plan.entries.map((entry) => entry.recipeId).filter((id): id is string => !!id),
        )
        .slice(-10) ?? [],
    [user?.plans],
  );
  const recommendation = useMemo(
    () =>
      profile && target && remainingCalories != null && remainingCalories >= 100
        ? recommendForNow({
        ...
        : null,
    [meals, profile, recentRecipeIds, remainingCalories, target],
  );
  ...
  useEffect(() => {
    ...
    void rankDailySuggestionCandidates(recommendation.candidates, {
      ...
      if (!cancelled) setRankedIds(ranked.map((recipe) => recipe.id));
    });
    return () => {
      cancelled = true;
    };
  ...
  const ranked = rankedIds
    ...
    .filter((recipe): recipe is (typeof recipeCatalog)[number] => !!recipe);
  const available = [...ranked, ...recommendation.candidates].filter(
    (recipe, index, recipes) =>
      recipes.findIndex((item) => item.id === recipe.id) === index &&
      !dismissedIds.includes(recipe.id),
  );
    ...
    <View style={styles.suggestionBlock}>
      ...
      {suggested.map((recipe) => (
        <DynamicMealCard
          ...
          imageUri={recipeImageUri(recipe)}
          nutrition={recipe.nutrition}
          detail={`${recipe.prepMinutes + recipe.cookMinutes} min / ${recipe.mealType}`}
          onPress={() => onOpenRecipe(recipe.id)}
          actions={[
            {
              icon: 'trash',
              label: `Not for me: ${recipe.name}`,
              tone: 'danger',
              onPress: () => {
            ...
            {
              icon: 'swap',
              label: `Show another ${recipe.mealType}`,
              onPress: () => setSuggestionCursor((current) => current + 1),
            },
            {
              icon: 'plus',
              label: `Log ${recipe.name}`,
              tone: 'primary',
              onPress: () => {
                setDismissedIds((current) => [...current, recipe.id]);
      ...
      <View style={styles.planHeader}>
        ...
        {plan ? (
          <View style={styles.planHeaderActions}>
            <Pressable onPress={onEditDay} style={styles.textButton} accessibilityLabel="Edit day">
              <Text style={[type.labelSm, { color: palette.primary }]}>Edit</Text>
            </Pressable>
            <Pressable
              onPress={onClearDay}
            ...
            >
              <Text style={[type.labelSm, { color: palette.onSurfaceVariant }]}>Clear</Text>
            </Pressable>
            <Pressable
              onPress={onDeletePlan}
              style={styles.iconAction}
              accessibilityLabel="Delete full plan"
            >
          ...
          <View style={styles.timeline}>
            {entries.map((entry) => (
              <DynamicMealCard
                ...
                imageUri={entry.imageUri ?? recipeImageUri(entry)}
                nutrition={entry.nutrition}
                detail={titleCase(entry.mealType)}
            ...
            {!entries.length ? (
              <EmptyState
                ...
                detail="Use Edit to add a meal, or leave the space for something spontaneous."
              />
            ) : null}
          </View>
        </>
...
function MealAction({
  ...
  return (
    <Pressable onPress={onPress} style={styles.mealAction} accessibilityRole="button">
      ...
      <Text
        style={[type.labelSm, { color: palette.onSurface, textAlign: 'center' }]}
        numberOfLines={2}
      >
        {label}
      </Text>
...
function confirmClearDay(
  plan: MealPlan,
  localDate: string,
  clear: (planId: string, date: string) => void,
) {
...
function confirmDeletePlan(plan: MealPlan, remove: (planId: string) => void) {
  Alert.alert(
    'Delete full plan?',
    'This removes every day in this plan. Your logged meals will not change.',
    [
      { text: 'Keep', style: 'cancel' },
