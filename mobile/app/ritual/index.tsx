...
export default function RitualScreen() {
  ...
  const purchaseCount =
    expenses.filter((expense) => expense.transactionDate === today).length +
  ...
  const todayJournalEntries = journalEntries.filter(
    (entry) => !entry.deletedAt && localDateKey(new Date(entry.writtenAt)) === today,
  );
      ...
      generateDailySignals(
        {
          ...
          plannedMeals,
          purchaseCount,
          pendingExpenseCount: expensePrompts.filter((prompt) => prompt.status === 'pending')
            .length,
          workout: {
          ...
          journal: {
            entryCount: todayJournalEntries.length,
            recentTags: journalEntries.flatMap((entry) => entry.tags).slice(0, 6),
          },
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      void writeDailyRitualSession(session);
    }, 300);
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      >
        {stage === 'entry' ? (
          <EntryStep
            signalCount={contextualSignals.length}
            onBack={goBack}
            onBegin={() =>
              beginSession(
                today,
                contextualSignals.map((signal) => signal.id),
              )
            }
          />
        ) : null}
        ...
        {stage === 'journal' ? (
          <View style={styles.stage}>
            <RitualHeading
              eyebrow="Optional · journal"
              title="Want to keep a little more?"
              description="Add one thought directly to Journal, or leave it here."
              onBack={goBack}
            />
            <JournalStep />
          </View>
        ...
        {stage === 'habits' ? (
          <View style={styles.stage}>
            <RitualHeading
              ...
              title="How did your commitments go?"
              description="Count what happened or skip only this day."
              onBack={goBack}
            />
            <HabitCheckin />
          </View>
        ...
        {stage === 'tomorrow' ? (
          <TomorrowStep
            habits={activeHabitsForDate(allHabits, nextLocalDate(today))}
            onBack={goBack}
            onDelete={(habit) => confirmFutureRemoval(habit, nextLocalDate(today), endHabit)}
            onSubstitute={(habit) =>
              router.push({
                pathname: '/habits/new',
                params: { replaceId: habit.id, effectiveDate: nextLocalDate(today) },
              })
            }
            onContinue={() => setStage('context')}
          />
        ) : null}
        {stage === 'context' ? (
          <ContextStep
            ...
            onBack={goBack}
            onFinish={finishTonight}
            onOpen={finishAndOpen}
          />
        ) : null}
      </ScrollView>
    </View>
...
function EntryStep({
  ...
  return (
    <View style={styles.stage}>
      ...
      <Text style={[type.bodySm, styles.centerNote]}>
        {signalCount
...
function PreviewLine({ number, label }: { number: string; label: string }) {
  return (
    <View style={styles.previewLine}>
      ...
      <Text style={[type.titleMd, styles.title]}>{label}</Text>
    </View>
  );
}
...
function TomorrowStep({
  ...
  onSubstitute: (habit: Habit) => void;
  onContinue: () => void;
}) {
  return (
    <View style={styles.stage}>
      <RitualHeading
        eyebrow="Then · tomorrow"
        title="What should stay with you?"
        description="Keep what fits, remove it from future days, or choose a substitute."
        onBack={onBack}
      />
      <View style={styles.tomorrowList}>
        {habits.map((habit) => (
          <View key={habit.id} style={styles.tomorrowRow}>
            <View style={styles.tomorrowName}>
              <Text style={[type.titleMd, styles.title]}>{habit.name}</Text>
              <Text style={[type.labelSm, styles.copy]}>
                {habit.category ?? 'Personal'} · {habit.schedule?.timeWindow ?? 'anytime'}
              </Text>
            </View>
            <Pressable
              onPress={() => onSubstitute(habit)}
              style={styles.smallAction}
              accessibilityRole="button"
              accessibilityLabel={`Substitute ${habit.name}`}
            >
              <Icon name="swap" size={20} color={palette.primary} />
            </Pressable>
            <Pressable
              onPress={() => onDelete(habit)}
              style={styles.smallAction}
              accessibilityRole="button"
              accessibilityLabel={`Delete ${habit.name} from future days`}
            >
              <Icon name="trash" size={20} color={palette.error} />
            </Pressable>
          </View>
        ))}
      </View>
...
function ContextStep({
  ...
  onBack: () => void;
  onFinish: () => void;
  onOpen: (signal: DailySignal) => void;
}) {
  return (
    <View style={styles.stage}>
      <RitualHeading
        eyebrow="Last · optional"
        title="Anything else before tomorrow?"
        description="Open one now, or finish the night. Neither choice leaves the ritual hanging."
        onBack={onBack}
      />
      {signals.length ? (
        <View style={styles.signalList}>
          {signals.map((signal) => (
            <SignalCard key={signal.id} signal={signal} onPress={() => onOpen(signal)} />
          ))}
        </View>
      ) : (
        <Card variant="recessed" style={styles.emptyCard}>
          <Text style={[type.headlineSm, styles.title]}>Nothing else needs your attention.</Text>
          <Text style={[type.bodySm, styles.copy]}>Your day is ready to close.</Text>
        </Card>
      )}
  ...
  return (
    ...
    >
      <View style={styles.signalIcon}>
        <Icon name={signalIcon(signal.source)} size={spacing.md} color={palette.primary} />
      </View>
      <View style={styles.signalBody}>
        <Text style={[type.labelSm, styles.copy]}>{signal.source}</Text>
...
function PrimaryButton({
  ...
  return (
    <Pressable
      onPress={onPress}
      disabled={disabled}
      style={({ pressed }) => [styles.primaryButton, (pressed || disabled) && styles.disabled]}
      accessibilityRole="button"
    >
...
function SecondaryButton({ label, onPress }: { label: string; onPress: () => void }) {
  return (
    <Pressable
      ...
      accessibilityRole="button"
    >
      <Text style={[type.labelMd, styles.title]}>{label}</Text>
    </Pressable>
  );
}
...
function confirmFutureRemoval(
  habit: Habit,
  effectiveDate: string,
  endHabit: (id: string, date: string) => void,
) {
  Alert.alert(
    'Delete from future days?',
    `${habit.name} will leave your schedule from tomorrow. Earlier days and completion history stay intact.`,
    [
      { text: 'Keep it', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => endHabit(habit.id, effectiveDate) },
    ],
  );
}

function signalIcon(source: DailySignal['source']): IconName {
  if (source === 'meals') return 'meals';
  if (source === 'money') return 'money';
  if (source === 'journal') return 'journal';
  if (source === 'music') return 'sparkles';
  if (source === 'commitments') return 'check';
  return 'health';
}
...
const styles = StyleSheet.create({
  ...
  timeLabel: { color: palette.primary, textAlign: 'center' },
  previewList: { flex: 1, gap: spacing.md },
  previewLine: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  centerNote: { color: palette.onSurfaceVariant, textAlign: 'center', padding: spacing.sm },
  loading: { minHeight: 320, alignItems: 'center', justifyContent: 'center', gap: spacing.md },
  emptyCard: { minHeight: 170, alignItems: 'center', justifyContent: 'center', gap: spacing.sm },
  reactionCard: { gap: spacing.sm },
  ...
  reactionChipSelected: { backgroundColor: palette.primary },
  moodCard: { minHeight: 300, justifyContent: 'center', gap: spacing.md },
  musicMood: { color: palette.primary },
  evidenceStrip: {
    padding: spacing.md,
    backgroundColor: palette.surfaceContainerHighest,
    borderLeftWidth: 4,
  ...
  evidenceCopy: { color: palette.onSurface, marginTop: spacing.xs },
  ...
  tomorrowName: { flex: 1, gap: spacing.xs },
  smallAction: { width: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' },
