  ...
  if (!active) {
    return (
      <View style={[styles.empty, { paddingTop: insets.top + spacing.lg }]}>
        <Icon name="health" size={32} color={palette.primary} />
        <Text style={[type.headlineMd, styles.primaryText]}>No workout is open</Text>
        <Text style={[type.bodyMd, styles.secondaryText]}>
          Choose a day from your Health plan, then tap Start workout.
        </Text>
        <ActionButton label="Back to Health" onPress={() => router.replace('/(tabs)/health')} />
      </View>
    );
  }
    ...
    return (
      ...
      >
        <View style={styles.doneMark}>
          <Icon name="check" size={32} color={palette.onPrimary} />
        </View>
        <SectionLabel>Workout complete</SectionLabel>
        <Text style={[type.displaySm, styles.primaryText, styles.center]}>
          {active.session.title}
        </Text>
        <Text style={[type.bodyMd, styles.secondaryText, styles.center]}>
          Saved to your movement history. Take a minute before the next thing.
        </Text>
        <View style={styles.finishStats}>
          <Stat value={`${progress.total}`} label="movements" />
          <Stat value={`${active.skippedStepIds.length}`} label="skipped" />
        </View>
        <ActionButton
          label="Done"
          onPress={() => {
    ...
    <View style={styles.root}>
      <View style={[styles.topbar, { paddingTop: insets.top + spacing.sm }]}>
        <Pressable
          ...
          accessibilityLabel="Pause and leave workout"
        >
          <Icon name="close" size={20} />
        </Pressable>
        <View style={styles.topbarCopy}>
          <SectionLabel>{active.session.title}</SectionLabel>
          <Text style={[type.bodySm, styles.secondaryText]}>
            {progress.completed} of {progress.total} movements done
          </Text>
        </View>
        <Text style={[type.labelSm, styles.accentText]}>
          {Math.round(((active.currentStepIndex + 1) / active.steps.length) * 100)}%
        </Text>
      </View>
      ...
      >
        <MovementPreview
          visualId={step.visualId ?? active.session.exercises[0]?.visualId ?? 'home_pushup'}
          ...
          <View style={styles.upNext}>
            <View>
              ...
              <Text style={[type.titleLg, styles.primaryText, styles.copyTop]}>
                {nextMeaningful.title}
              </Text>
              <Text style={[type.bodySm, styles.secondaryText]}>{nextMeaningful.prescription}</Text>
            </View>
            {nextMeaningful.visualId ? (
              <ExerciseVisual visualId={nextMeaningful.visualId} style={styles.nextImage} />
            ) : null}
          </View>
        ) : null}
      </ScrollView>
      ...
      <View style={[styles.controls, { paddingBottom: insets.bottom + spacing.md }]}>
        <Pressable
          ...
          disabled={active.currentStepIndex === 0 && currentSet === 1}
          style={[
            styles.smallControl,
            active.currentStepIndex === 0 && currentSet === 1 && styles.disabled,
          ]}
          accessibilityRole="button"
        >
          <Text style={[type.labelMd, styles.primaryText]}>Back</Text>
        </Pressable>
        {step.mode === 'timer' ? (
          <Pressable
            onPress={active.isPaused ? resumeWorkout : pauseWorkout}
            style={styles.mainControl}
            accessibilityRole="button"
            accessibilityLabel={active.isPaused ? 'Resume timer' : 'Pause timer'}
          >
        ...
        <Pressable
          ...
          accessibilityRole="button"
        >
          <Text style={[type.labelMd, styles.accentText]}>
            {step.kind === 'rest' ? 'End rest' : 'Skip'}
          </Text>
        </Pressable>
      </View>
    </View>
...
function MovementPreview({ visualId, animate }: { visualId: string; animate: boolean }) {
  ...
  return (
    ...
    >
      <ExerciseVisual visualId={visualId} style={styles.visual} />
    </Animated.View>
  );
}
...
function TimerDial({
  ...
  return (
    <View style={styles.timerWrap}>
      ...
      <Text style={[type.displayMd, styles.timerText]}>{formatClock(remaining)}</Text>
      <Text style={[type.labelSm, styles.timerLabel]}>{paused ? 'Paused' : 'Remaining'}</Text>
    </View>
  );
}
...
function ActionButton({ label, onPress }: { label: string; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={styles.actionButton} accessibilityRole="button">
      <Text style={[type.labelMd, styles.mainControlText]}>{label}</Text>
    </Pressable>
  );
}
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <View style={styles.stat}>
      <Text style={[type.displaySm, styles.primaryText]}>{value}</Text>
      <Text style={[type.labelSm, styles.secondaryText]}>{label}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  timerText: { color: palette.onSurface },
  timerLabel: { color: palette.onSurfaceVariant, marginTop: -4 },
  manualCount: {
    minHeight: 130,
    alignItems: 'center',
    justifyContent: 'center',
  ...
  mainControlText: { color: palette.onPrimary },
  ...
  finishStats: { width: '100%', flexDirection: 'row', gap: spacing.sm, marginVertical: spacing.md },
  stat: {
    flex: 1,
    alignItems: 'center',
    padding: spacing.md,
