...
export function HabitCheckin() {
  ...
  const habits = useProductionStore((state) => state.habits.filter((habit) => !habit.archivedAt).sort((left, right) => left.position - right.position));
  ...
  return (
    <View style={styles.container}>
      <View style={styles.introCard}>
        ...
        <Text style={[type.headlineSm, styles.title]}>Count what happened. Pause what did not fit.</Text>
      </View>

      <View style={styles.list}>
        {habits.length ? habits.map((habit) => {
          const done = habit.completedOn.includes(today);
          const paused = habit.pausedOn?.includes(today) ?? false;
          return (
            <View key={habit.id} style={styles.row}>
              ...
              <Pressable onPress={() => pause(habit.id)} style={[styles.pauseButton, paused && styles.pauseButtonActive]} accessibilityRole="switch" accessibilityState={{ checked: paused }} accessibilityLabel={`${paused ? 'Resume' : 'Pause'} ${habit.name} today`}>
                <Text style={[type.labelSm, paused ? styles.accent : styles.copy]}>{paused ? 'Paused' : 'Pause'}</Text>
              </Pressable>
            </View>
          );
      ...
      <View style={styles.recoveryNote}>
        ...
        <Text style={[type.bodySm, styles.copy]}>An intentional pause keeps the rhythm honest.</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  pauseButton: { minWidth: spacing['3xl'], minHeight: spacing['2xl'], alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.sm, borderRadius: radii.pill, backgroundColor: palette.surfaceContainerHighest },
  pauseButtonActive: { backgroundColor: palette.primaryContainer },
